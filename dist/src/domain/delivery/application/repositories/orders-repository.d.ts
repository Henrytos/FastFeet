...
export declare abstract class OrdersRepository {
    ...
    abstract fetchOrdersByRecipientId(recipientId: string, page: number): Promise<Order[]>;
    abstract fetchManyNearby(coordinate: Coordinate, page: number): Promise<Order[]>;
    abstract fetchManyNearbyWithDistance(coordinate: Coordinate, page: number): Promise<OrderWithDistance[]>;
    abstract deleteManyByRecipientId(recipientId: string): Promise<void>;
    abstract save(order: Order): Promise<void>;
    abstract delete(order: Order): Promise<void>;
    abstract fetchOrderByDeliveryManId({ deliveryManId, page, perPage, }: {
        deliveryManId: string;
        page: number;
        perPage: number;
    }): Promise<Order[]>;
    abstract fetchRecentOrder({ page, perPage, }: {
        page: number;
        perPage: number;
    }): Promise<Order[]>;
